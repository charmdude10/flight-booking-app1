...
const SeatMap = ({ selectedSeats, onSeatSelect }) => {
  ...
  const rows = Array.from(new Set(seats.map((seat) => seat.row))).sort(
    (a, b) => a - b
  ); // Generates an array of numbers from 1 to 30
        ...
        <div className="relative">
          ...
          <div className=" absolute -left-10 top-0 bottom-0 flex flex-col justify-around">
            ...
            {rows.map((row) => (
              <div
                key={`row-${row}`}
                className="h-10 flex items-center justify-center text-sm font-medium text-foreground/70"
              >
                {row}
              </div>
            ))}
                      ...
                      <button
                        ...
                        className={`
                                ...
                                ${
                                  status === "available" &&
                                  seat.type === "standard" &&
                                  "bg-gray-200 hover:bg-gray-300/30 dark:bg-gray-700 dark:hover:bg-travel/50"
                                },
                                ${
                                  status === "available" &&
                                  seat.type === "extra-legroom" &&
                                  "bg-blue-200 hover:bg-blue-300 dark:bg-blue-900/50 dark:hover:bg-blue-800/60"
                                },
                                ${
                                  status === "available" &&
                                  seat.type === "window" &&
                                  "bg-purple-200 hover:bg-purple-300 dark:bg-purple-900/50 dark:hover:bg-purple-800/60"
                                },
                                ${
                                  status === "available" &&
                                  seat.type === "aisle" &&
                                  "bg-red-200 hover:bg-purple-300 dark:bg-red-600/50 dark:hover:bg-red-800/60"
                                },
                                ${
                                  status === "occupied" &&
                                ...
                                ${
                                  status === "selected" &&
                                  "bg-blue-900 hover:bg-gray-900 text-white"
                                }
                              `}
                      >
                        {status === "occupied" ? (
