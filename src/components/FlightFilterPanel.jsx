...
const FlightFilterPanel = ({ filters, setFilters }) => {
  ...
  const [travelTimeRange, setTravelTimeRange] = useState(
    filters?.travelTime || [0, 1440]
  );
  ...
  const handleAirlineChange = (value) => {
    setFilters((prev) => {
      const airlines = prev.airlines.includes(value)
        ? prev.airlines.filter((item) => item !== value)
        : [...prev.airlines, value];
  ...
  const handleStopsChange = (value) => {
    setFilters((prev) => {
      const stops = prev.stops.includes(value)
        ? prev.stops.filter((item) => item !== value)
        : [...prev.stops, value];
      return { ...prev, stops };
    });
  };
  ...
  const handleCabinClassChange = (value) => {
    setFilters((prev) => {
      const cabinClass = prev.cabinClass.includes(value)
        ? prev.cabinClass.filter((item) => item !== value)
        : [...prev.cabinClass, value];
      ...
      <div className="flex flex-col gap-2 mt-4">
        ...
        <div className="pt-2">
          <RangeSlider
            ...
            className="w-full h-2  rounded-lg appearance-none cursor-pointer my-4"
          />

          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-600">
              {formatToNairaShort(priceRange[0])}
            </span>
            <span className="text-sm font-medium text-gray-600">
              {formatToNairaShort(priceRange[1])}
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="pt-2">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm font-medium text-gray-500">
              {formatTravelTime(travelTimeRange[0])}
            </span>
            <span className="text-sm font-medium text-gray-500">
              {formatTravelTime(travelTimeRange[1])}
            </span>
          </div>
        </div>
      </div>
